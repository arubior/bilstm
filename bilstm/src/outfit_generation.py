...
from create_mosaic import create_img
...
def main(model_name, feats_name, img_savepath, cuda):
    ...
    for i, outfit in enumerate(fitb):
        ...
        create_img(outfit, pred[0].data[0], os.path.join(img_savepath, "%d_score%.2f.jpg" % (i, pred[1].data[0]*100)))
        scores.append(pred)
...
def main_single_prev(model_name, feats_name, img_savepath, cuda):
    """Main function."""
    outfit_file = 'data/label/fill_in_blank_test.json'
    fitb = json.load(open(outfit_file))
...
if __name__ == '__main__':
    ...
    # main_single_prev(ARGS.model_path, ARGS.feats_path, ARGS.img_path, ARGS.cuda)
    main(ARGS.model_path, ARGS.feats_path, ARGS.img_path, ARGS.cuda)
