...
from src.model_squeezenet import FullBiLSTM
...
IMG_TRF = {'train': ImageTransforms(305, 5, 227, 0.5),
           'test': ImageTransforms(227)}
...
IMG_TEST_VAL_TF = lambda x: TRF(torchvision.transforms.ToTensor()(IMG_TRF['test'].resize(x)))
...
def train(train_params, dataloaders, cuda, batch_first, epoch_params):
    ...
    log_name = ('runs/squeezenet/lr%.3f' % optimizer.param_groups[0]['initial_lr'])
    if freeze:
        log_name += '_frozen'
