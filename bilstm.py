...
class FC_BiLSTM_CRF(nn.Module):
    ...
    def __init__(self, vocab_size, tag_to_ix, embedding_dim, fc_dim, hidden_dim):
        ...
        self.fc = nn.Sequential(nn.Linear(embedding_dim, 20),
                                nn.Linear(20, fc_dim))
        self.lstm = nn.LSTM(fc_dim, hidden_dim // 2,
    ...
    def _get_lstm_features(self, sentence):
        ...
        embeds_fc = self.fc(embeds)
        lstm_out, self.hidden = self.lstm(embeds_fc, self.hidden)
...
for i, data in enumerate(dataloaders['train']):
    if i < 5:
        ...
        for idx, lab in enumerate(labels):
            if lab is 0:
                kimgs.append(inputs[idx])
                ktags.append(lab)
            else:
                limgs.append(inputs[idx])
                ltags.append(lab)
...
model = FC_BiLSTM_CRF(len(word_to_ix), tag_to_ix, EMBEDDING_DIM, FC_DIM, HIDDEN_DIM)
